...
serverN=${1:-batt}
docker run --rm --volumes-from "$serverN" -v ${d}:/backup ubuntu tar cvf /backup/battdata.tar /batt/server/data
gzip ${d}battdata.tar
...
docker run --rm --volumes-from "$archN" -v ${d}:/backup ubuntu tar cvf /backup/archdata.tar /arch/data
gzip ${d}archdata.tar
